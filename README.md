...
| [Type Dataset](#4.1.20-type-dataset)                                               | `adms:representationTechnique`  | IRI                                                           | 0...1       | R             | [INSPIRE Spatial representation type](https://inspire.ec.europa.eu/metadata-codelist/SpatialRepresentationType)                                                                                             |
...
| [Subject](#4124-subject)                                                           | `dcterms:subject`               | IRI                                                           | 0...n       | O             | A FAVV-specific controlled vocabulary (Recommended), [INSPIRE Topic categories in accordance with EN ISO 19115](https://inspire.ec.europa.eu/metadata-codelist/TopicCategory)                                                                                    |
...
- **Predefined values**: [INSPIRE Spatial Representation Types](https://inspire.ec.europa.eu/metadata-codelist/SpatialRepresentationType) (Recommended)
- **Notes**: The form must validate that a well-formed IRI is given according [RFC3987](https://www.ietf.org/rfc/rfc3987.txt). Ideally it should be a dereference-able IRI.
  Since this property is specifically used to map INSPIRE-based geospatial service metadata to DCAT, it is not relevant for FAVV. Therefore, the possible values can be left empty.
  If the controlled list of possible values is empty, the form might omit this field from rendering.
- **Example**:
  ```json
  {
...
- **Predefined values**: A FAVV-specific controlled vocabulary (Recommended), [INSPIRE Topic categories in accordance with EN ISO 19115](https://inspire.ec.europa.eu/metadata-codelist/TopicCategory)
- **Notes**: The form must validate that a well-formed IRI is given according [RFC3987](https://www.ietf.org/rfc/rfc3987.txt). Ideally it should be a dereference-able IRI.
  In the context of FAVV, the predefined list of possible values should most likely be different from the INSPIRE topic categories, and could be based on the internal classification of FAVV datasets. In that case, the form should use a FAVV-specific controlled vocabulary.
  Values must be of type `skos:Concept` and have at least the properties `skos:prefLabel` and `skos:inScheme` (pointing to the controlled vocabulary).
  If the controlled list of possible values is empty, the form might omit this field from rendering.
- **Example**:
  ```json
  {
